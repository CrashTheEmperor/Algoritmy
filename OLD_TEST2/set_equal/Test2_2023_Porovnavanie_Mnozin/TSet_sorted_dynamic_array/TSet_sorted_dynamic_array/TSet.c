 ...
 *  \author Petyovský
 *  \version 2025
 *  $Id: TSet.c 3085 2025-04-17 18:24:24Z petyovsky $
 */
...
bool set_is_equal(const struct TSet* aLeftSet, const struct TSet* aRightSet) {
	...
	struct TSetIterator it= set_iterator_begin(&aLeftSet);
	set_iterator_is_valid(&it);
	

	for (size_t i = 0; i < aLeftSet->iSize; i++) {
		if(!set_is_element_of(aRightSet, set_iterator_value(aLeftSet)))
			return false;
