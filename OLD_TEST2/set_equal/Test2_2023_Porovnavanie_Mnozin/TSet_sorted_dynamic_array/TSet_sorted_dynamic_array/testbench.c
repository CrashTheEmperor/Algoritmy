 ...
 *  \author Petyovský
 *  \version 2025
 *  $Id: testbench.c 3085 2025-04-17 18:24:24Z petyovsky $
 */
#include <stdio.h>
#include <time.h>
#include "testbench.h"
...
void print_value_by_iterator(const TTestedADTIterator *aIter)
	{
	if(!set_element_store_file(set_iterator_value(aIter), stdout))
...
static bool test_run(TTestedADT *aADT, const struct TTestPlan *aTestPlan)
	...
	if(aTestPlan->iListOperations)
		...
		for(size_t i = 0; i < aTestPlan->iInputs.iSize; ++i)
			...
			if(!set_element_store_file(aTestPlan->iInputs.iValues[i], stdout))
				fputs("Function 'vector_element_store_file()' failed!\n", stderr);
