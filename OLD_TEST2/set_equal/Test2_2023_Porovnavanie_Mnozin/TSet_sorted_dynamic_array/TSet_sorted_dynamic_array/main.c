...
int main(int argc, char* argv[])
{
	...
	struct TSet set3 = { 0 };
	set_init(&set3);
	struct TSetIterator it1 = set_iterator_begin(&set1);
	struct TSetIterator it2 = set_iterator_begin(&set2);
	struct TSetIterator it3 = set_iterator_begin(&set3);

	set_iterator_is_valid(&it1);
	set_iterator_is_valid(&it2);
	set_iterator_is_valid(&it3);
