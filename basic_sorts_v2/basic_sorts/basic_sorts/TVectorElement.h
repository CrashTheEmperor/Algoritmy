 ...
 *  \author Petyovský
 *  \version 2025
 *  $Id: TVectorElement.h 2938 2025-02-13 16:08:51Z petyovsky $
 */
 ...
 */
[[nodiscard]] static inline int vector_element_comparator(const TVectorElement *aLeft, const TVectorElement *aRight)
	{
	assert(aLeft);
	assert(aRight);
	if(*aLeft == *aRight)
 ...
 *  \attention Funkce ověřuje platnost obou ukazatelů \b pouze při překladu v režimu `Debug`, kdy pomocí `assert` hlásí běhovou chybu!
 */
[[nodiscard]] static inline bool vector_element_load_file(TVectorElement *aElement, FILE *aInputFile)
	{
	assert(aElement);
	assert(aInputFile);
 ...
 */
[[nodiscard]] static inline bool vector_element_store_file(TVectorElement aElement, FILE *aOutputFile)
	{
	assert(aOutputFile);
 ...
 */
[[nodiscard]] static inline TVectorElement vector_element_random_value(void)
	{
