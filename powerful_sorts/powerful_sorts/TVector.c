 ...
 *  \author Petyovský
 *  \version 2025
 *  $Id: TVector.c 3044 2025-04-04 18:05:14Z petyovsky $
 */
...
bool vector_clone(const struct TVector *aVector, struct TVector *aVectorClone)
	...
	assert((!aVectorClone->iValues && !aVectorClone->iSize) || (aVectorClone->iValues && aVectorClone->iSize));
	...
	*aVectorClone = (struct TVector) { .iValues = act_values, .iSize = aVector->iSize, .iStats = stats, .iStatsAttached = stats_attached };
	return true;
	}
...
[[nodiscard]] static inline TVectorElement vector_fast_value(const struct TVector *aVector, size_t aPos)
	{
...
[[nodiscard]] static inline int vector_fast_compare_positions(const struct TVector *aLeftVector, size_t aLeftPos, const struct TVector *aRightVector, size_t aRightPos)
	{
...
[[nodiscard]] static inline int vector_fast_compare_position_value(const struct TVector *aVector, size_t aPos, TVectorElement aValue)
	{
...
bool vector_iterator_to_prev(struct TVectorIterator *aIter)
	...
	if(aIter && aIter->iVector)
		{
		if(aIter->iPos && aIter->iPos < aIter->iVector->iSize)
			{
			--aIter->iPos;
			return true;
			}
...
bool vector_iterator_is_equal(const struct TVectorIterator *aLeftIter, const struct TVectorIterator *aRightIter)
	{
	const bool left_iter_is_valid = vector_iterator_is_valid(aLeftIter);
	const bool right_iter_is_valid = vector_iterator_is_valid(aRightIter);
...
int vector_iterator_comparator(const struct TVectorIterator *aLeftIter, const struct TVectorIterator *aRightIter)
	{
	const bool left_iter_is_valid = vector_iterator_is_valid(aLeftIter);
	const bool right_iter_is_valid = vector_iterator_is_valid(aRightIter);
	...
	assert(aLeftIter->iVector == aRightIter->iVector);

	if(aLeftIter->iPos == aRightIter->iPos)
		return 0;
	if(aLeftIter->iPos < aRightIter->iPos)
		return -1;
	return 1;
