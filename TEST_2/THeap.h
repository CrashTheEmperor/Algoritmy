 ...
 *  \author Petyovský
 *  \version 2025
 *  $Id: THeap.h 3067 2025-04-11 17:20:21Z petyovsky $
 */
 ...
 */
[[nodiscard]] static inline size_t heap_decrement_size(struct THeap *aHeap)
	{
	assert(aHeap);
 ...
 *  \attention Funkce ověřuje platnost ukazatele \p aHeap \b pouze při překladu v režimu `Debug`, kdy pomocí `assert` hlásí běhovou chybu!
 */
[[nodiscard]] static inline size_t heap_restore_size(struct THeap *aHeap)
	{
	assert(aHeap);
 ...
 */
[[nodiscard]] static inline TVectorElement heap_value(const struct THeap *aHeap, size_t aPos)
	{
