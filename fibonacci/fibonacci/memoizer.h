 ...
 *  \author Petyovský
 *  \version 2025
 *  $Id: memoizer.h 3039 2025-03-28 16:01:33Z petyovsky $
 */
 ...
 */
[[nodiscard]] static inline bool memoizer_init(struct TMemoizer *aM, size_t aCapacity, TMemoizedFunc *aMemoizedFunc)
	{
 ...
 */
[[nodiscard]] static inline TMemoizedFuncOutput memoizer_eval(struct TMemoizer *aM, TMemoizedFuncInput aInput)
	{
	assert(aM);
	assert(aM->iArchive);
 ...
 */
[[nodiscard]] static inline bool memoizer_print_stats(const struct TMemoizer *aM, FILE *aOutputFile)
	{
	if(!aM || !aOutputFile)
		return false;
 ...
 */
[[nodiscard]] static inline bool memoizer_clear(struct TMemoizer *aM)
	{
	if(!aM)
		return false;
